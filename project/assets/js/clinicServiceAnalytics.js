...
function listOfClinicFeedbacks(){
    let feedbackList = [];
            ...
            if(response.success) {
                for(let i = 0; i < response.data.length; i++){
                    if(response.data[i].office.toLowerCase() === 'clinic'){
                        ...
                        analyzeClinicFeedbackWithAI(feedbackList, function(err, results, gwa) {
                            if (err) {
                                jQuery('#clinicServiceFeedbackMostCommonAnswer').html(`
                                    <tr><td colspan="3" class="text-center text-danger">Failed to analyze feedback.</td></tr>
                                `);
                                return;
                            }
                            jQuery('#clinicServiceFeedbackMostCommonAnswer').empty();
...
function analyzeClinicFeedbackWithAI(feedbacks, callback) {
    jQuery.ajax({
