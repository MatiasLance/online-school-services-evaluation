            ...
            if(response.success) {
                for(let i = 0; i < response.data.length; i++){
                    if(response.data[i].office.toLowerCase() === 'maintenance'){
                        ...
                        analyzeMaintenanceFeedbackWithAI(feedbackList, function(err, results, gwa) {
                            if (err) {
                                jQuery('#maintenanceServiceFeedbackMostCommonAnswer').html(`
                                    <tr><td colspan="3" class="text-center text-danger">Failed to analyze feedback.</td></tr>
                                `);
                                return;
                            }
                            jQuery('#maintenanceServiceFeedbackMostCommonAnswer').empty();
...
function analyzeMaintenanceFeedbackWithAI(feedbacks, callback) {
    jQuery.ajax({
