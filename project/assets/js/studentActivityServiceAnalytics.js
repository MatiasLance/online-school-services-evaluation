            ...
            if(response.success) {
                for(let i = 0; i < response.data.length; i++){
                    if(response.data[i].office.toLowerCase() === 'student activity'){
                        ...
                        analyzeSecurityFeedbackWithAI(feedbackList, function(err, results, gwa) {
                            if (err) {
                                jQuery('#studentActivityServiceFeedbackMostCommonAnswer').html(`
...
function analyzeStudentActivityFeedbackWithAI(feedbacks, callback) {
    jQuery.ajax({
