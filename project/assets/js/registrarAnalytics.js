            ...
            if(response.success) {
                for(let i = 0; i < response.data.length; i++){
                    if(response.data[i].office.toLowerCase() === 'registrar'){
                            ...
                            analyzeRegistrarFeedbackWithAI(feedbackList, function(err, results, gwa) {
                                if (err) {
                                    jQuery('#registrarServiceFeedbackMostCommonAnswer').html(`
...
function analyzeRegistrarFeedbackWithAI(feedbacks, callback) {
    jQuery.ajax({
